...
class FormComponent extends Component
{
    ...
    public function siguientePregunta(){
        if($this->ae_status == 1){
            ...
            $this->var_msg = false;
            $this->ae_status = $this->ae_status + 1;
        }else if($this->ae_status == 17){
            if($this->ae_urlTicket != null){
                $this->var_msg = false;
                $this->ae_status = $this->ae_status + 1;
            }else{
                $this->var_msg_error = "Adjuntar el comprobante del deposito inicial es obligatorio";
                $this->var_msg = true;
            }
        }else{
            $this->ae_status = $this->ae_status + 1;
    ...
    public function finalizarForm(){
        ...
        $this->validate([
            'ae_urlTicket' => 'image|mimes:jpeg,jpg,png|max:2048', // 1MB Max
        ]);
        $this->ae_urlTicket->storeAs('comprobantes', $this->ae_cedula.'.jpg', 'public');
        ...
        $vl_registro->urlTicket = '/storage/comprobantes/'.$this->ae_cedula.'.jpg';
        $vl_registro->save();
        $this->ae_status = $this->ae_status + 1;
        Log::info('Pregunta #'.$this->ae_status);
    }
